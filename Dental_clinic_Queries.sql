...
FROM client c join appointment a on c.VAT = a.VAT_client
    ...
    join employee e on d.VAT = e.VAT, phone_number_client p_c where e.name
    like 'Jane Sweettoth' and a.date_timestamp <= NOW()
    and c.VAT = p_c.VAT
...
SELECT DISTINCT e.name as traine_name, e.VAT, e_d.name as doctor_name,
    sr.evaluation, sr.description
FROM employee e
